...
def b_wave(x):
    if x <= 4 * np.pi / mode:
        return B_amp * np.sin(x * mode)
    return 0

def db_dx_wave(x):
    if x <= 4 * np.pi / mode:
        return B_amp * mode * np.cos(mode*x)
    return 0
...
def s_depth(COMP, show_det = False):
    ...
    jA_ec = np.zeros_like(xx, dtype = float)
    for  i , x in enumerate(xx):
        jA_ec[i] = - db_dx_wave(x) * np.sin(psi) * B_norm / unit_ch
    #jA_ec = - db_dx_wave(xx) * np.sin(psi) * B_norm / unit_ch
    jA_ec_max = np.max(np.abs(jA_ec))
    n_p_tot, n_m_tot = 0, 0
    for i, x in enumerate(xx):
        n_0_local = jA_ec_max * bg_density_scaled(x)
        k = jA_ec[i] / n_0_local
        beta_p, beta_m = solve_for_beta(k)
        n_p, n_m = n_0_local/(1 - beta_p), n_0_local/(1 - beta_m)
        n_p_tot += n_p
        n_m_tot += n_m
    weight = (n_p_tot + n_m_tot)/(ppc0 * grid_x)
    skin_depth = COMP / np.sqrt(weight)
    if show_det:
        print("jA_ec_max = ", jA_ec_max)
        print("weight = ", weight)
        print("skin_depth = ", skin_depth)
        print("target skin_depth = ", wA_wp * 2 * np.pi / mode)
    return skin_depth
...
with warnings.catch_warnings(action="ignore"):
    ...
    s_depth(COMP_true, show_det = True)
    print(COMP_true)
