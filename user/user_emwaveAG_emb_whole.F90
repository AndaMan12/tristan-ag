module m_userfile
  ...
  real, private :: jA_ec_max, weight, wA_wp
...
contains
  ...
  subroutine userReadInput()
    ...
    call getInput('problem', 'wA_wp', wA_wp)
    ...
    init_x_boundary = int(2 * M_PI / mode) + 1
  ...
  subroutine bg_density_scaled (mult1, mult2, sx_glob, xg, n_0_local)
    ...
    if (mult1 .eq. mult2) then
      ...
      k = log((mult1-mult2) / (0.01 * mult2))/ (sx_glob - init_x_boundary)
